...
private val faqList = listOf(
    ...
    FaqListItem.Entry(FaqItem(
        "「导入模式」是什么？模式1 和模式2 有什么区别？",
        "导入模式决定课表数据从哪里来，直接影响上课时间与教室的取值口径：\n\n" +
            "模式1（默认）——解析[个人课表]和[周次课表]，逐周下载后合并。时间和教室以周次课表为准，教师以个人课表为准。数据更完整，但要一学期一学期地下载，稍慢。\n\n" +
            "模式2——只解析[个人课表]，不下载周次课表，稍快。它依赖的接口更少，是备用线路：教务的周次课表接口临时不可用时，模式1 会整体失败，改用模式2 通常仍能导入成功。\n\n" +
            "和历史学期的关系：每个学期在缓存时会记下当时用的模式。重新下载历史学期会改用你当前选中的模式，同一门课的时间或教室可能与上次不一样——所以按下「重新下载」之前，页面会提示「该学期原用模式X缓存，重新下载将改用模式Y」。历史学期不会自动更新。\n\n" +
            "和首页刷新的关系：首页刷新只作用于当前学期，并按你当前选中的模式重新抓取；查看历史学期时点刷新会提示「历史学期为只读缓存，无需刷新」。想更新历史学期，请到「导入课表」页单独重新下载。"
    )),
    ...
    FaqListItem.Entry(FaqItem(
        "课时统计是怎么算出来的？",
        "单节课时长 × 生效周数，逐课次累加。\n\n" +
            "单节课时长：只算每一节本身的时间，不含课间。例如「第 3-4 节」= 第 3 节时长 + 第 4 节时长，两节之间的休息不计入。\n\n" +
            "生效周数：按课次的周次实际生效的周数算。「单周」「双周」只算一半；「全周」或没写周次，按整学期周数算。\n\n" +
            "教师维度：一门课有多位老师时，每位老师各计满一份，所以教师合计会大于课程合计，这是有意的。\n\n" +
            "只统计已下载的学期。学期周数缺失、作息不完整、或课次周次无法解析的学期会被整体排除，并在页面上提示重新下载。当前学期用你当前的作息设置（改了上课时间立刻生效），历史学期用导入时保存的作息。"
    )),
