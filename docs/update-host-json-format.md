...
本文说明 `C:\Users\ZhuanZ\Desktop\app-update-host` 仓库中三个线上 JSON 文件的维护方式：

- `update.json`：用于 App 版本更新检查。
- `notices.json`：用于 App 内“通知”菜单。
- `greetings.json`：用于 Android 侧边栏问候语，并作为微信端后续同步契约。

三个文件都会通过 Cloudflare Pages 发布到 `https://update.999314.xyz/`。
